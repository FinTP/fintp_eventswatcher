...
class Service
{
	public :
		...
		// ctor
		Service() : m_State( Service::STOPPED ), m_LastHeartbeat( 0 ), m_ServiceName( "qPIUnk00" ), m_SessionId( "" ),
			m_ServiceId( -1 ), m_HeartbeatInterval( 30 ) {};

		Service( const long serviceId, const long state, const time_t lastHeartbeat, 
			const string& serviceName="qPIUnk00", const string& sessionId="", const long heartbeatInterval = 30 ) :
