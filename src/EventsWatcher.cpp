...
string Service::Report() const
{
	...
	reportMessage << "Service [" << m_ServiceName << "] with id [" << m_ServiceId << "] is " <<
		ToString( m_State ) << ". Last heartbeat was on " << TimeUtil::Get( "%d/%m/%Y %H:%M:%S", 20, &m_LastHeartbeat );
...
{	
	AppSettings GlobalSettings = AppSettings( configFile );
...
{	
	...
	try
	{
		...
		{	
			long serviceId = states->getCellValue( i, "SERVICEID" )->getLong();
			long state = states->getCellValue( i, "STATUS" )->getLong();
			string serviceName = StringUtil::Trim( states->getCellValue( i, "FRIENDLYNAME" )->getString() );
...
void EventsWatcher::UploadMessage( AppException& ex )
{
	...
	EventsWatcherDbOp::InsertEvent( serviceId, ex.getCorrelationId(), sessionId,
		...
		ex.getMessage(), "no class", ex.getAdditionalInfo()->ToString(), innerException );
